...
`total: number` - общая стоимость товаров в корзине
`state: boolean` - состояние корзины
...
`set state` - активирует/деактивирует кнопку оформления заказа, добавляет информацию о пустой корзине в контейнер в зависимости от наличия товаров в корзине
